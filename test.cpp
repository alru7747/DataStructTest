...
void LinkedList::insert_begin(int newKey){
  //1. ALLOCATE NODE
  Node* newNode = new Node;//creating a new node, new memory space
...
void LinkedList::insert_end(int newKey){
  ...
  Node* temp = head;//unlike above where we create a new node, we are just setting this node to head
...
bool LinkedList::deleteAtIndex(int n)
{
  ...
  if(n==0){
    ...
    Node *delnode=temp->next;//temp->next referst to the link to the next value in the linked list
